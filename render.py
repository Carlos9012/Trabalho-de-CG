"""render.py – plotting helpers"""
import numpy as np
import matplotlib.pyplot as plt
from mpl_toolkits.mplot3d.art3d import Line3DCollection, Poly3DCollection
from collections import defaultdict
from algebra import (
    apply_transform, camera_R_T, perspective_matrix, project_ndc_to_screen
)
...
def show_individual_solids(
    solids, meshes, *,
    show_faces=True, show_wire=True, show_mesh=False,
    palette=PALETTE,
):
    """One window per solid (local space)."""
    ...
    for idx, (name, verts, edges) in enumerate(solids, 1):
        ...
        if name != "line" and show_faces:
            ...
            if len(mf):
                ...
                ax.add_collection3d(
                    Poly3DCollection(tris, facecolors=color, edgecolors="none", alpha=0.65)
                )

        if name != "line" and show_mesh:
            mv, mf = mesh_lookup[name]
            mesh_lines = [[mv[a], mv[b]] for a, b, c in mf] + \
                         [[mv[b], mv[c]] for a, b, c in mf] + \
                         [[mv[c], mv[a]] for a, b, c in mf]
            ax.add_collection3d(
                Line3DCollection(np.array(mesh_lines), colors=color, lw=0.6)
            )

        if show_wire or name == "line":
            segs = np.array([[verts[i], verts[j]] for i, j in edges])
            ax.add_collection3d(Line3DCollection(segs, colors=color, lw=0.8))

        bb_min, bb_max = verts.min(0), verts.max(0)
        ctr, rad = (bb_min + bb_max) / 2, max(bb_max - bb_min) / 2 or 1.0
...
def show_scene(
    ...
    """World‑space view of all solids."""
    segments_always, segments_cond, facesets = [], [], []

    for idx, (name, verts, edges) in enumerate(solids):
        col = palette[idx % len(palette)]
        vw = apply_transform(verts, transforms[name])

        if name == "line":
            segments_always += [[[vw[i], vw[j]], col] for i, j in edges]
            continue

        if show_wire:
            segments_cond += [[[vw[i], vw[j]], col] for i, j in edges]

        mv, mf = next((v, f) for n, v, f in meshes if n == name)
        if len(mf):
            mv_w = apply_transform(mv, transforms[name])
    ...
    if show_faces:
        for tris, col in facesets:
            ax.add_collection3d(
                Poly3DCollection(tris, facecolors=col, edgecolors='none', alpha=0.6)
            )
    ...
    if show_mesh:
        ...
        if mesh_lines:
            ax.add_collection3d(
                Line3DCollection(np.array(mesh_lines), colors=mesh_cols, lw=0.6)
            )
...
def show_camera_scene(
    ...
    """Camera‑space view with frustum."""
    ...
    for idx, (name, verts, edges) in enumerate(solids):
        ...
        if name == "line":
            segments_always += [[[vc[i], vc[j]], col] for i, j in edges]
            continue

        if show_wire:
            segments_cond += [[[vc[i], vc[j]], col] for i, j in edges]
    ...
    N = cn + np.array([[-h_n/2, -h_n/2, 0], [h_n/2, -h_n/2, 0],
                       [h_n/2, h_n/2, 0], [-h_n/2, h_n/2, 0]])
    F = cf + np.array([[-h_f/2, -h_f/2, 0], [h_f/2, -h_f/2, 0],
                       [h_f/2, h_f/2, 0], [-h_f/2, h_f/2, 0]])
